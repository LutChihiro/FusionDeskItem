...
public class EvaluateCommand implements Runnable {
    ...
    @Override public void run(){var cases=new EvaluationCaseLoader().load();var report=new EvaluationRunner(OpenAiCompatibleLlmProvider.fromEnvironment()).run(promptVersion,cases,System.getenv("LLM_MODEL"));Path output=new EvaluationReportWriter(Path.of("evaluation-results")).write(report);long runId=new com.xfusion.fusiondesk.repository.EvaluationRepository(root.database()).saveEvaluationRun(report);EvaluationMetrics m=report.metrics();
